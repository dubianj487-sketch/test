...
export default function CastPage() {
  ...
  const nav = (
    <div style={{ position: 'fixed', bottom: 0, left: '50%', transform: 'translateX(-50%)', width: '100%', maxWidth: 390, zIndex: 40, background: '#ffffff', borderTop: '1px solid #efefef', padding: '10px 14px 24px', display: 'flex', justifyContent: 'space-around', boxSizing: 'border-box' }}>
      <div onClick={() => setScreen('home')} role="button" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4, cursor: 'pointer', color: screen === 'home' ? '#0a0a0a' : '#b5b5b5' }}>
        ...
        <span style={{ fontSize: rem(10.5), fontWeight: 700 }}>ホーム</span>
      </div>
      <div onClick={() => setScreen('place')} role="button" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4, cursor: 'pointer', color: screen === 'place' ? '#0a0a0a' : '#b5b5b5' }}>
        ...
        <span style={{ fontSize: rem(10.5), fontWeight: 700 }}>降車場所</span>
      </div>
      <div onClick={() => setScreen('request')} role="button" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4, cursor: 'pointer', color: screen === 'request' ? '#0a0a0a' : '#b5b5b5' }}>
        ...
        <span style={{ fontSize: rem(10.5), fontWeight: 700 }}>申請</span>
      </div>
    </div>
  )
  ...
  if (screen === 'home') return (
    <div style={{ minHeight: '100dvh', background: '#fff', color: '#0a0a0a', padding: '52px 0 110px', boxSizing: 'border-box', animation: 'lm-fade .3s ease both', fontFamily: font }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 20px 14px' }}>
        <div>
          <p style={{ margin: 0, fontSize: rem(12), fontWeight: 600, color: '#8a8a8a' }}>CLUB VENUS・KING</p>
          <h1 style={{ margin: '2px 0 0', fontSize: rem(22), fontWeight: 800, letterSpacing: '-.02em' }}>こんばんは、{castGirl.name}さん</h1>
        </div>
      ...
      <div style={{ padding: '0 20px' }}>
        {castEntry ? (
          <div style={{ background: '#0a0a0a', borderRadius: 20, padding: 20, color: '#fff' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <span style={{ fontSize: rem(13), fontWeight: 700, color: '#cfcfcf' }}>本日の帰り便 ・ {castTripStatus}</span>
            </div>
            <div style={{ marginTop: 16, display: 'flex', alignItems: 'flex-end', gap: 6 }}>
              <span style={{ fontSize: rem(44), fontWeight: 800, lineHeight: 1, letterSpacing: '-.02em' }}>{castEntry.dropNo}</span>
              <span style={{ fontSize: rem(15), fontWeight: 600, color: '#a8a8a8', paddingBottom: 6 }}>/ {castTrip?.assignedIds.length} 番目に降車</span>
            </div>
            <div style={{ marginTop: 18, display: 'flex', alignItems: 'center', gap: 12, background: '#1a1a1a', borderRadius: 13, padding: 12 }}>
              <div style={{ width: 40, height: 40, borderRadius: '50%', background: '#333', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: rem(15), flexShrink: 0 }}>{castDrv?.initial || '−'}</div>
              <div style={{ flex: 1 }}><p style={{ margin: 0, fontSize: rem(14), fontWeight: 700 }}>{castDrv?.name || '未定'}</p><p style={{ margin: '1px 0 0', fontSize: rem(12), color: '#9a9a9a' }}>{castDrv?.car || '−'}</p></div>
              <span style={{ fontSize: rem(13), fontWeight: 700, color: '#06c167', whiteSpace: 'nowrap', flexShrink: 0 }}>{castTrip?.departTime} 出発</span>
            </div>
          </div>
        ) : (
          <div style={{ background: '#f7f7f7', borderRadius: 18, padding: 20 }}>
            <p style={{ margin: '0 0 14px', fontSize: rem(14), color: '#8a8a8a', textAlign: 'center' }}>本日の帰り便にはまだ登録されていません。</p>
            {app.rideRequests[castId] === 'approved' ? (
              <div style={{ background: '#eafaf0', border: '1px solid #bdeccf', borderRadius: 12, padding: '12px 14px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  ...
                  <p style={{ margin: 0, fontSize: rem(14), fontWeight: 700, color: '#0a7a3f' }}>今日の送迎をリクエストしました</p>
                </div>
                <p style={{ margin: '6px 0 0 28px', fontSize: rem(12), color: '#3a8a5a', lineHeight: 1.5 }}>乗車情報が確定したらこちらに表示されます。</p>
              </div>
            ) : (
              <button onClick={sendRideRequest} style={{ width: '100%', height: 52, borderRadius: 13, background: '#0a0a0a', color: '#fff', border: 'none', fontSize: rem(15), fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
        ...
        {castTodayReq && castTodayReq.status !== '承認済み' && (
          <div style={{ marginTop: 12, background: '#fff8ed', border: '1px solid #ffe3b8', borderRadius: 14, padding: 14 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <span style={{ fontSize: rem(11), fontWeight: 700, background: '#ffe3b8', color: '#8a5a00', padding: '2px 8px', borderRadius: 999 }}>{castTodayReq.status}</span>
            </div>
            <p style={{ margin: '6px 0 0', fontSize: rem(13.5), fontWeight: 600, color: '#8a5a00' }}>{castTodayReq.place}</p>
          </div>
        )}

        <p style={{ margin: '24px 4px 10px', fontSize: rem(13), fontWeight: 700, color: '#8a8a8a', letterSpacing: '.04em' }}>降車場所</p>
        <div onClick={() => setScreen('place')} role="button" style={{ border: '1px solid #ededed', borderRadius: 16, padding: 16, display: 'flex', alignItems: 'center', gap: 12, cursor: 'pointer' }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <p style={{ margin: '2px 0 0', fontSize: rem(14), fontWeight: 600, lineHeight: 1.4 }}>{castDrop}</p>
          </div>
        ...
        <button onClick={() => setScreen('request')} style={{ marginTop: 14, width: '100%', height: 56, borderRadius: 15, background: '#0a0a0a', color: '#fff', border: 'none', fontSize: rem(15.5), fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
  ...
  if (screen === 'place') return (
    <div style={{ minHeight: '100dvh', background: '#fff', color: '#0a0a0a', padding: '52px 0 110px', boxSizing: 'border-box', animation: 'lm-fade .3s ease both', fontFamily: font }}>
      ...
      <div style={{ padding: '0 20px' }}>
        <p style={{ margin: '6px 4px 16px', fontSize: rem(13.5), color: '#7a7a7a', lineHeight: 1.6 }}>普段、帰りに降りる場所を登録します。送迎の降車ルートはこの住所をもとに組まれます。</p>
        <p style={{ margin: '0 4px 8px', fontSize: rem(12), fontWeight: 700, color: '#8a8a8a', letterSpacing: '.04em' }}>住所・目印</p>
        <textarea
          value={dropDraft}
          onChange={e => { setDropDraft(e.target.value); setDropSaved(false) }}
          placeholder="例：新潟市中央区万代1-6-3 ○○マンション402"
          style={{ width: '100%', height: 120, borderRadius: 14, background: '#f7f7f7', border: '1px solid #e6e6e6', color: '#0a0a0a', padding: '14px 16px', fontSize: rem(15), fontFamily: 'inherit', lineHeight: 1.6, outline: 'none', boxSizing: 'border-box', resize: 'none' }}
        />
        <p style={{ margin: '10px 4px 0', fontSize: rem(12), color: '#a0a0a0', lineHeight: 1.5 }}>※ マンション名・部屋番号・近くの目印まで入れると、ドライバーが迷いません。</p>
        ...
        <button onClick={saveDrop} style={{ marginTop: 24, width: '100%', height: 56, borderRadius: 15, background: '#0a0a0a', color: '#fff', border: 'none', fontSize: rem(16), fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>この場所を保存</button>
      </div>
      {nav}
    </div>
  )
    ...
    <div style={{ minHeight: '100dvh', background: '#fff', color: '#0a0a0a', padding: '52px 0 110px', boxSizing: 'border-box', animation: 'lm-fade .3s ease both', fontFamily: font }}>
      ...
      <div style={{ padding: '0 20px' }}>
        <div style={{ background: '#0a0a0a', borderRadius: 16, padding: 16, color: '#fff', display: 'flex', gap: 12, alignItems: 'flex-start' }}>
          ...
          <p style={{ margin: 0, fontSize: rem(13.5), lineHeight: 1.6, color: '#dcdcdc' }}>今日だけ、いつもと違う場所で降りたいときに申請できます。ボーイが確認後に降車ルートへ反映されます。</p>
        </div>

        <p style={{ margin: '22px 4px 8px', fontSize: rem(12), fontWeight: 700, color: '#8a8a8a', letterSpacing: '.04em' }}>本日の降車場所</p>
        <input
          value={reqPlace}
          onChange={e => setReqPlace(e.target.value)}
          placeholder="例：新潟駅 南口 タクシー乗り場前"
          style={{ height: 54, width: '100%', borderRadius: 14, background: '#f7f7f7', border: '1px solid #e6e6e6', color: '#0a0a0a', padding: '0 16px', fontSize: rem(15), fontFamily: 'inherit', outline: 'none', boxSizing: 'border-box' }}
        />

        <p style={{ margin: '18px 4px 8px', fontSize: rem(12), fontWeight: 700, color: '#8a8a8a', letterSpacing: '.04em' }}>理由（任意）</p>
        <textarea
          value={reqReason}
          onChange={e => setReqReason(e.target.value)}
          placeholder="例：万代で友人と待ち合わせのため"
          style={{ width: '100%', height: 90, borderRadius: 14, background: '#f7f7f7', border: '1px solid #e6e6e6', color: '#0a0a0a', padding: '14px 16px', fontSize: rem(15), fontFamily: 'inherit', lineHeight: 1.6, outline: 'none', boxSizing: 'border-box', resize: 'none' }}
        />

        <button onClick={submitToday} style={{ marginTop: 24, width: '100%', height: 56, borderRadius: 15, background: '#0a0a0a', color: '#fff', border: 'none', fontSize: rem(16), fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>この内容で申請する</button>
        <p style={{ margin: '12px 4px 0', fontSize: rem(12), color: '#a0a0a0', textAlign: 'center' }}>申請は本日の送迎のみに適用されます。</p>
      </div>
      {nav}
    </div>
  )
