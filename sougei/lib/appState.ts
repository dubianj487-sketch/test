export type GirlKey = 'kotone' | 'miki' | 'yuna' | 'ai' | 'rena' | 'saki' | 'marin'
export type DriverKey = 'sato' | 'tanaka'
...
export type AppState = {
  trips: Trip[]
  nextTripId: number
  rideRequests: Partial<Record<GirlKey, 'approved'>>
  todayRequests: Partial<Record<GirlKey, TodayRequest>>
  driverStatuses: Record<DriverKey, string>
  castDrops: Record<GirlKey, string>
}

export const DEFAULT_CAST_DROPS: Record<GirlKey, string> = {
...
export function loadAppState(): AppState {
  ...
  try {
    ...
    if (!raw) return { ...DEFAULT_APP_STATE, castDrops: { ...DEFAULT_CAST_DROPS } }
    ...
    return { ...DEFAULT_APP_STATE, castDrops: { ...DEFAULT_CAST_DROPS } }
  }
}
...
export const GIRLS: Record<GirlKey, Girl> = {
...
export const DRIVERS: Record<DriverKey, DriverInfo> = {
...
export function buildTripObjs(trip: Trip, completed?: number) {
  const done = completed ?? trip.completed ?? 0
  const objs = trip.assignedIds
    .map(id => ({ id: id as GirlKey, ...GIRLS[id as GirlKey] }))
